...
- `npm run audio:prepare` converts the exact bot-running master in `audio/bot_running/` to a 48 kHz Opus loop, the exact mechanical master in `audio/turning_loop/` to a 48 kHz WAV, and the six `audio/bot_stepping/` samples to processed 48 kHz WAV footsteps. The footstep pass adds restrained pitch wobble, digital crushing, phaser color, compression, and limiting before loudness normalization. It writes committed browser assets under `public/audio/` and regenerates `src/lib/audio-assets.ts`. FFmpeg and FFprobe must be available on `PATH` when regenerating assets.
...
- `StreetViewController.onSuccessfulStep` is the sole footstep trigger. Each successful panorama move plays one complete processed bot-step sample from the shuffle pool with restrained runtime pitch and gain variation.
- `/bot` uses one fixed Piper voice for every review: `PIPER_VOICE_INDEX` in `src/lib/piper-config.ts`, currently `1` (`en_US-amy-medium`). Live TTS settings are centralized in `TTS` in `src/lib/config.ts`: Piper length scale `1.0`, pre-voice hold `900 ms`, subtitle lead/lag `0 ms`, and subtitle linger `3500 ms`. Do not rotate voices in the live kiosk path unless the artwork direction changes.
